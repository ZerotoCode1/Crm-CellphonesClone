...
const useGetListOrderEsim = (filters: RequestListOrderEsim, refetchKey: string) => {
  ...
  const fetch = async () => {
    const body = {
      ...requestBaseApi({ wsCode: WS_CODE.LIST_ORDER_ESIM }),
      wsRequest: {
        ...filters,
      },
    };
...
export default useGetListOrderEsim;
